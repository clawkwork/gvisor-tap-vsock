...
func UDP(s *stack.Stack, nat map[tcpip.Address]tcpip.Address, natLock *sync.Mutex, ec2MetadataAccess bool, filter AddressFilter) *udp.Forwarder {
	return udp.NewForwarder(s, func(r *udp.ForwarderRequest) bool {
		...
		if filter != nil {
			if err := filter(addr); err != nil {
				log.Tracef("UDP filter rejected %s: %v", addr, err)
				return true
			}
		}
